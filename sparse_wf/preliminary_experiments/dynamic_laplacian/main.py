...
if __name__ == "__main__":
    ...
    layers = [folx.forward_laplacian(layer) for layer in layers]
    ...
    print(
        f"{n_el} electrons, each with {n_neighbours} neighbours: Dense input dim = (1 + {n_neighbours}) * 3 = {3 * (n_neighbours + 1)}"
    )
    ...
    for ind_layer, layer in enumerate(layers):
        ...
        print(
            "Jac input shapes: h={h.jacobian.data.shape}, h_neighbour={h_neighbour.jacobian.data.shape}, diff={diff_neighbour.jacobian.data.shape}"
        )
