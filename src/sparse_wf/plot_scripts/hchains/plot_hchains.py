...
# Relative quantities
ax_rel.errorbar(
    df_rel.cutoff, df_rel.rel_error * 1000, df_rel.rel_error_sigma * 1000, color=COLOR_FIRE, capsize=3, label=None
)
...
for idx_method, method in enumerate(ref_methods):
    ...
    ax_rel.text(
        0.3,
        error_rel + (-0.05 if "AFQMC" in method else 0.05),
        method,
        va="top" if "AFQMC" in method else "bottom",
        ha="left",
        color=color,
    )
