...
energies = jax.tree.map(
    lambda r: pd.DataFrame(r.scan_history(keys=["opt/E", "opt/E_std", "opt/step"])).set_index("opt/step").sort_index(),
    runs,
...
line_styles = ["solid", "dashed", "dotted", "dashdot", (0, (5, 1)), (0, (3, 5, 1, 5, 1, 5))]
...
ax.fill_between(
    range(len(order)),
    reference.loc["ZPE-corr'd exp"][order] - 1.6,
    reference.loc["ZPE-corr'd exp"][order] + 1.6,
    color="black",
    alpha=0.1,
    zorder=-10,
    label="exp $\pm$ chem. acc",
)
ax.plot(
    reference.loc["CCSD(T)/FPA"][order], "^", color=colors[1], label="CCSD(T)/FPA", zorder=3, linestyle=line_styles[2]
)
ax.plot(
    reference.loc["ACI-DSRG-MRPT2"][order],
    "v",
    color=colors[2],
    label="ACI-DSRG-MRPT2",
    zorder=4,
...
ax2.barh(
    pos := pos - w,
    pd.Series(final_deltas)[order] - reference.loc["ZPE-corr'd exp"][order],
    height=w,
    color=colors[0],
    label="FiRE",
    zorder=10,
...
ax2.barh(
    pos := pos - w,
    reference.loc["CCSD(T)/FPA"][order] - reference.loc["ZPE-corr'd exp"][order],
    height=w,
    color=colors[1],
    label="CCSD(T)/FPA",
    zorder=5,
...
ax2.barh(
    pos := pos - w,
    reference.loc["ACI-DSRG-MRPT2"][order] - reference.loc["ZPE-corr'd exp"][order],
    height=w,
    color=colors[2],
    label="ACI-DSRG-MRPT2",
    zorder=6,
...
ax2.barh(
    pos := pos - w,
    reference.loc["AFQMC"][order] - reference.loc["ZPE-corr'd exp"][order],
    height=w,
    color=colors[3],
    label="AFQMC",
    zorder=4,
...
ax2.errorbar(
    ...
    x + (n + 1) / 2 * w - w,
    xerr=errors[order],
    color=scale_lightness(colors[0], 0.7),
    capsize=2,
    label="FiRE",
    linestyle="",
    zorder=10,
...
ax2.errorbar(
    ...
    pos,
    xerr=afqmc_error,
    color=scale_lightness(colors[3], 0.7),
    capsize=2,
    label="AFQMC",
    linestyle="",
    zorder=5,
...
our = our.rename(
    columns={"triplet": r"$E^\text{FiRE}_\text{triplet}$ (Ha)", "singlet": r"$E^\text{FiRE}_\text{singlet}$ (Ha)"}
)
our = our.dropna().T
...
for ax, k in zip(axes, energies_kcal_per_mol.keys()):
    ...
    ax.fill_between([x.min(), x_max], exp - 1.6, exp + 1.6, color="black", alpha=0.1, label="exp $\pm$ chem. acc ")
...
final_deltas = {
    k: (v["triplet"] - v["singlet"]) * 1000 for k, v in final_energies.items() if "singlet" in v and "triplet" in v
}
...
for (i, k), ax in zip(enumerate(energies_kcal_per_mol), axes):
    ...
    ax.bar(
        1,
        reference[k]["CCSD(T)/FPA"] - reference[k]["ZPE-corr'd exp"],
        width=w,
        color=next(c_iter),
        label="CCSD(T)/FPA",
    )
    ax.bar(
        2,
        reference[k]["ACI-DSRG-MRPT2"] - reference[k]["ZPE-corr'd exp"],
        width=w,
        color=next(c_iter),
        label="ACI-DSRG-MRPT2",
    )
...
print(
    "CCSD(T)/FPA MAE:",
    np.mean(np.abs([reference[k]["CCSD(T)/FPA"] - reference[k]["ZPE-corr'd exp"] for k in energies_kcal_per_mol])),
)
print(
    "ACI-DSRG-MRPT2 MAE:",
    np.mean(np.abs([reference[k]["ACI-DSRG-MRPT2"] - reference[k]["ZPE-corr'd exp"] for k in energies_kcal_per_mol])),
)
print(
    "AFQMC MAE:",
    np.nanmean(np.abs([reference[k]["AFQMC"] - reference[k]["ZPE-corr'd exp"] for k in energies_kcal_per_mol])),
)
