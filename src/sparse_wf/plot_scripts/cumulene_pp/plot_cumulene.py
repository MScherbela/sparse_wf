...
pivot["deltaE_groundtruth"] = pivot.n_carbon.map(
    pivot[pivot.method == "PBE0 CCSD(T) / CBS"].set_index("n_carbon")["deltaE"]
)
...
for idx_n_carbon, n_carbon in enumerate(n_carbon_values):
    ...
    ax.plot(pivot.index / 1000, pivot[90], label="90 deg", color="dimgray")
    ax.plot(pivot.index / 1000, pivot[0], label="0 deg", color="black")
    ax.set_ylim([pivot[0].iloc[-1] - 5e-3, pivot[90].iloc[-1] + 20e-3])
    ax.yaxis.get_major_formatter().set_useOffset(False)
    twinx.plot(pivot.index / 1000, pivot["deltaE"], label="deltaE", color=COLOR_FIRE)
