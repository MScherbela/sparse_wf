...
class MoonElecEmb(nn.Module):
    ...
    def __call__(
        self,
        r: Float[Array, "dim=3"],
        e_emb: Float[Array, "feature_dim"],  # noqa: F821
        r_nb: Float[Array, "*neighbours dim=3"],
        e_nb_emb: Float[Array, "*neighbours feature_dim"],
        s: Optional[Int] = None,
...
class GatedLinearUnit(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        dense_layer = nn.Dense(self.features * 2, use_bias=False)
        # Split into two parts, one for output and one for gate
        linear_output, gate_output = jnp.split(dense_layer(x), 2, axis=-1)
        # Apply sigmoid to gate part
...
class Moon(MoonLikeWaveFunction):
    def setup(self):
        ...
        self.gamma_en_init = MoonNucToElecGamma(
            R=self.R,
            cutoff=self.cutoff,
            filter_dims=self.pair_mlp_widths,
            feature_dim=self.feature_dim,
    ...
    def _embedding(self, electrons: Electrons, static: StaticInput) -> Electrons:
        ...
        # construct electron from nuclei
        gamma_en_init, _, edge_en_emb_init = self.gamma_en_init(electrons, R_nb_en, idx_nb.en)
        edge_en_emb = nn.silu(edge_en_emb_init)
        h00 = contract(edge_en_emb, gamma_en_init)
        h00 = self.elec_mlp(h00)

        # construct electron from electrons
        h00_nb = get_with_fill(self.elec_nb_proj(nn.silu(h00)), idx_nb.ee, 0)
        h0 = nn_vmap(self.elec_elec_emb)(
            electrons,
            h00,
            r_nb_ee,
            h00_nb,
