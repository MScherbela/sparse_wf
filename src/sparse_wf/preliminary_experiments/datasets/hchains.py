...
spacing = 2.8
Z = [1] * n_atoms
R = [[i*spacing, 0, 0] for i in range(n_atoms)]
name = f"HChain{n_atoms}"
comment = f"HChain{n_atoms}_{spacing:.2f}"
geom = Geometry(Z=Z, R=R, name=name, comment=comment)
save_geometries({geom.hash: geom})
