...
runs = [r for r in runs if r.name.startswith("mpconf_WG") or r.name.startswith("mpconf_hf_v2_WG")]
...
for r in runs:
    ...
    metadata = dict(
        ...
        run_name=r.name,
        model="hf_like" if "mpconf_hf" in r.name else "swann")
...
plots = []
for model, title in zip(["hf_like", "swann"], ["HF-like model", "Full SWANN"]):
    ...
    p = bpl.figure(width=600, height=600, title=f"Relative Energies vs CCSD(T): {title}", x_axis_label="Optimization Step", y_axis_label="E - E_mean / mHa", tools="box_zoom,hover,save", tooltips=[("step", "$x"), ("delta", "$y")])
...
grid = bokeh.layouts.gridplot(plots, ncols=2)
bpl.output_notebook()
bpl.show(grid)
