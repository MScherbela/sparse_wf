...
PERIODIC_TABLE = "H He Li Be B C N O F Ne Na Mg Al Si P S Cl Ar".split()
    ...
    with open("energies.csv", "w", buffering=1) as energy_file:
        ...
        for basis_set in args.basis_set:
            for geom_hash in args.geometries:
                ...
                t0 = time.time()
                results = run_orca(
                    g, directory, method, basis_set, args.frozen_core, args.n_proc, args.total_memory, args.orca_path
                )
                t1 = time.time()
                energy_file.write(
                    f"{ind_calc};{geom_hash};{geom_comment};{method_string};{basis_set};{results.get('E_hf', '')};{results.get('E_final', '')};{t1-t0}\n"
                )
                ind_calc += 1
