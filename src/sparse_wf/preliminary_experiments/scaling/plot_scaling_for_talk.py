...
# t_total_swann = df_speedup.loc["FiRE"].t_sampling + df_speedup.loc["FiRE"].t_E_kin
...
for i, model in enumerate(models):
    ax_speedup.text(i, df_speedup.loc[model].sum()+1, f"{df_speedup.loc[model].sum() / t_total_swann:.0f}$\\times$", ha="center", va="bottom")
