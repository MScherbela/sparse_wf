...
include = df_ref.basis_set.str.contains("TZ") | df_ref.method.str.contains("DLPNO")
...
for idx_n_carbon, n_carbon in enumerate(df_full["n_carbon"].unique()):
    ...
    for cutoff, color in zip(cutoffs, colors):
        ...
        ax.plot(pivot.index, pivot[90], label=f"90 deg, c={cutoff}", color=color)
        ax.plot(pivot.index, pivot[0], label=f"0 deg, c={cutoff}", color=color)
        ax.set_ylim([pivot[0].iloc[-1]-5e-3, pivot[90].iloc[-1]+20e-3])
        twinx.plot(pivot.index, pivot["deltaE"], label=f"deltaE, c={cutoff}", color='k', linestyle="--")
