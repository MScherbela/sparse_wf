...
for g, E_fit, slope, color in zip(geoms, E_fit_values, slopes, COLOR_PALETTE[:4]):
    ax_ext.scatter(pivot_smooth[("grad", g)], pivot_smooth[("E", g)], c=pivot_smooth.index, clim=(pivot_smooth.index.min(), pivot_smooth.index.max()))
...
df_last = pivot["E"].dropna().ffill(limit=10).rolling(smoothing_last).mean().iloc[smoothing_last :: smoothing_last // 10]
