...
def plot_bar(ax, x, energies, method, **kwargs):
    E = energies[method]
    # if E == 0:
    #     ax.plot([0, 0], [x-BAR_WIDTH/2, x+BAR_WIDTH/2], **kwargs, lw=3)
    #     return
    ax.barh([x], [E], height=BAR_WIDTH, **kwargs)
    ax.text(0.1, x, f"{E:.1f}", ha="left", va="center", color="white")



def plot_delta_bar(ax, x, energies, method1, method2, **kwargs):
    E1, E2 = energies[[method1, method2]]
    ax.barh([x], [E2 - E1], left=E1, height=BAR_WIDTH, color="dimgray", **kwargs)
    plt.plot([E1, E1], [x - 0.5 * BAR_WIDTH, x - 1 + 0.5 * BAR_WIDTH], color="k", lw=0.7)
    plt.plot([E2, E2], [x + 0.5 * BAR_WIDTH, x + 1 - 0.5 * BAR_WIDTH], color="k", lw=0.7)
...
x_inset, y_inset, h_inset = 0.4, 0.07, 0.53
...
methods_inset = methods[:-1] + [("composite", "Conventional\nbest est.", "k")] + methods[-1:]
width_per_bar = BAR_WIDTH / len(methods_inset)
for idx_m, (m, label, color) in enumerate(methods_inset):
    # label=label.split(":\n")[0].replace("\n", " ")
    label = "conv.\nbest\nestimate" if m == "composite" else None
    ax_inset.bar(
        np.arange(3) - 0.5 * BAR_WIDTH + idx_m * width_per_bar,
        df.loc[m],
        label=label,
        color=color,
        width=width_per_bar,
...
ax_inset.set_xticks(np.arange(3))
ax_inset.set_xticklabels(df.columns)
ax_inset.set_ylabel("$E - E_\\text{HC}$ " + MILLIHARTREE, labelpad=2)
ax_inset.grid(False, axis="x")
ax_inset.legend(loc="upper left", handlelength=0.4, handleheight=3.5, handletextpad=0.5, frameon=False, bbox_to_anchor=(-0.02, 1.05))
...
methods_tex = [methods_inset[-2]] + [("FiRE", "FiRE, $c=5a_0$, raw", None), ("FiRE_ext", "FiRE, $c=5a_0$, extrapolated", None)] + methods_inset[:-2]
df_tex = df.copy()
df_tex.insert(0, "HC", 0)

uncertainty_fire_rel = np.sqrt(uncertainty_fire**2 + uncertainty_fire["HC"]**2).drop("HC")
...
# # ax.barh([0], [mae[methods[0][0]]], label=methods[0][1])
# plot_bar(ax, 0, mae, methods[0], color=COLOR_PALETTE[0])
# plot_delta_bar(ax, 1, mae, methods[0], methods[1], label="corrections")
# plot_delta_bar(ax, 2, mae, methods[1], methods[2])
# plot_delta_bar(ax, 3, mae, methods[2], methods[3])
# plot_bar(ax, 4, mae, methods[3], color=COLOR_PALETTE[1])
# plot_delta_bar(ax, 5, mae, methods[3], methods[4])
# plot_bar(ax, 6, mae, methods[4], color=COLOR_PALETTE[2])

# # ax.axvspan(0, uncertainty_fire, color=COLOR_FIRE, alpha=0.5, zorder=-1)
# # ax.errorbar(mae[methods[-2:]], [3, 5], xerr=np.array([8, np.sqrt(8**2 + 10**2)]) * KJ_PER_MOL_IN_mHA, color="dimgray", ls="none")
...
# ax.set_xlim([0, 18.5])
# ax.set_ylim([6.5, -1.2])
# ax.set_xlabel("mean absolute deviation vs. FiRE " + MILLIHARTREE)
# ax.grid(False, axis="y")
# ax.yaxis.minorticks_off()
# ax.legend(loc="upper right")

# ax_inset = ax.inset_axes((0.39, 0.07, 0.6, 0.45))

# methods_inset = [
#     ("UHF/CCSD/TZ", "CCSD/TZ", COLOR_PALETTE[0]),
#     ("UHF/CCSD(T)/CBS_23", "CCSD(T)/CBS", COLOR_PALETTE[1]),
#     # ("UHF/CCSD(T)/TZ",  "CCSD(T)/TZ", COLOR_PALETTE[3]),
#     ("composite", "conv. best est.", COLOR_PALETTE[2]),
#     ("FiRE", "FiRE, $c=5a_0$", COLOR_FIRE),
# ]
...
# ax_inset.set_xticks(np.arange(3))
# ax_inset.set_xticklabels(df.columns)
# ax_inset.set_ylabel("$E - E_\\text{HC}$ " + MILLIHARTREE, labelpad=0)
# ax_inset.grid(False, axis="x")

# handles, labels = ax_inset.get_legend_handles_labels()
# fig.legend(handles, labels, loc="upper center", ncol=4, handlelength=1.2, handletextpad=0.5, columnspacing=1.2)
# fig.tight_layout()
# fig.subplots_adjust(top=0.92)
# savefig(fig, "Fe2S2_waterfall")
