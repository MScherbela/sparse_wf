...
def get_cumulene(N, phi_in_deg):
    ...
    R_hydrogen[1] = R_left + CH_bond * np.array([np.cos(theta), np.sin(theta)*np.cos(0+np.pi), np.sin(theta)*np.sin(0+np.pi)])
    R_hydrogen[2] = R_right - CH_bond * np.array([np.cos(theta), np.sin(theta)*np.cos(phi), np.sin(theta)*np.sin(phi)])
    R_hydrogen[3] = R_right - CH_bond * np.array([np.cos(theta), np.sin(theta)*np.cos(phi+np.pi), np.sin(theta)*np.sin(phi+np.pi)])
    return np.concatenate([R_carbon, R_hydrogen]), np.concatenate([np.ones(N) * 6, np.ones(4)])

R, Z = get_cumulene(4, 90)

atoms = Atoms(positions=R / ANGSTROM_IN_BOHR, numbers=Z)
view(atoms)
