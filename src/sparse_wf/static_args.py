...
class StaticScheduler:
    def __init__(
        ...
        self.step = 0
        self.did_recompile = jnp.ones(history_length, jnp.bool)
    ...
    def __call__(self, actual_static: StaticInput, compilation_cache_size: int | None = None) -> StaticInput:
        ...
        if compilation_cache_size is not None:
            ...
            if jnp.sum(self.did_recompile) <= 3:
                padding_factor = self.min_padding_factor
            else:
                padding_factor = self.max_padding_factor
        else:
            padding_factor = self.max_padding_factor

        static = jtu.tree_map(lambda x: int(jnp.max(x)), self.history)
        static = static.round_with_padding(padding_factor, self.n_electrons, self.n_up, self.n_nuclei)
        return static
